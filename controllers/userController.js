...
export default {
 ...
 update: catchAsync(async (req, res, next) => {
  ...
  UserSchema.findOneAndUpdate({ _id: id }, updateData)
   .then((data) => {
    ...
    const role = data.role === 'student' ? 'user' : 'teacher';
 ...
 changePassword: catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.curPW, user.password))) {
    req.session.pwWrong = true;
    const role = user.role === 'student' ? 'user' : 'teacher';
  ...
  for (var i = 0; i < buyCrs.length; i++) {
   ...
   await courseModel.findOne({ _id: buyCrs[i].idCourse }).then((course) => {
    ...
    for (var j = 0; j < course.listChapter.length; j++) {
     ...
     if (tmp.toString() === currentChapter.toString()) {
      //Đã xem tới chương này rồi
